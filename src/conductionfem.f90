...
! End added for omp
	...
	subroutine finitesolution()
		...
		double precision,dimension(:),allocatable :: kvals,bvals,gF,Tvals
		double precision,dimension(:,:),allocatable :: ver,gK
...
!8. Routine to manipulate the final global stiffness and global force matrices for solution
	subroutine setupfinalequations(gK,Tvals,gF)
		double precision,dimension(:,:),intent(inout) :: gK
		double precision,dimension(:),intent(inout) :: Tvals,gF
		...
		!$omp parallel &
		!$omp shared (Tvals,knT,unT,gK,gF) &
		!$omp private (i)
...
end module conductionfeminclude 'mkl_pardiso.f90'	! Added for pardiso
